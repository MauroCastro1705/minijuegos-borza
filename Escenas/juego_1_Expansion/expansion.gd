...
func level_up():
	...
	tween.parallel().tween_property(planeta, "friction", planeta.friction + 10, duration)\
		.set_ease(ease_type).set_trans(Tween.TRANS_QUAD)
	
	tween.parallel().tween_property(planeta, "acceleration", planeta.acceleration - 5, duration)\
		.set_ease(ease_type).set_trans(Tween.TRANS_QUAD)
	
	tween.parallel().tween_property(planeta, "scale", planeta.scale + Vector2(0.5, 0.5), duration)\
